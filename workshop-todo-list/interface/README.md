...
> We've already seen an interface provided by Angular: the `OnInit` interface which includes the method `ngOnInit`. Every Class that implements this interface should define this method. Otherwise we'll get an error during compile time. Note: in modern Angular, the CLI no longer generates `OnInit` by default in new components, but you can still add it when you need initialization logic.
>
...
{% hint style="success" %}
[See the results on StackBlitz](https://stackblitz.com/github/dominika-zajac/todo-list-tutorial/tree/master/examples/0\_11-interface)
{% endhint %}
